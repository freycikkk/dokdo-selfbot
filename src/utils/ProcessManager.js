...
/**
 * @typedef Action
 * @property {Discord.MessageButton} button // won't work for selfbots, selfbots can't have buttons
...
module.exports = class ProcessManager {
  ...
  async init() {
    ...
    console.log(this.filterSecret(this.messageContent));
  }
  ...
  async addAction(actions, args) {
    ...
    // Removing the button logic since selfbots can't use buttons
    ...
    // Selfbots can't interact with buttons, so we remove that functionality here
  }

  async createMessageComponentMessage() {
    // No buttons will be created for selfbots
    if (this.options.noCode && this.splitted.length < 2) return;
    // Simply skip creating any button components
  }
  ...
  edit() {
    // Buttons are not created for selfbots, so no button handling needed
  ...
  destroy() {
    // No buttons to remove or stop for selfbots
    this.message.edit({ components: [] });
  }
