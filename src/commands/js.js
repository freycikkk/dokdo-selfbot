...
module.exports = async function js(message, parent) {
    ...
    .then(async (output) => {
      ...
      async function prettify(target) {
        if (
          ...
          await message.channel.send({ embeds: [target] });
        else if (isinstance(target, Discord.Attachment)) {
          await message.channel.send({
            files:
              target instanceof Discord.Collection ? target.array() : [target],
          });
        }
      }
      ...
      if (isGenerator(output)) {
        for (const value of output) {
          ...
          if (typeof value === "function")
            await message.channel.send(value.toString());
  ...
  const fileBuffer = Buffer.from(result.toString(), "utf-8");
  const attachment = new Discord.MessageAttachment(fileBuffer, "output.txt");
